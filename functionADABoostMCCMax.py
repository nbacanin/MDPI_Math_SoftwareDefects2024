# ADABOOST funkcija za feature selection i tuning
# koristi se default classificatior DecisionTree pa se radi optimizacija DT max_depth
...
class ADABOOSTFunction:
    def __init__(self, D, no_classes, nfeatures, fitness_type, alpha, x_train, x_test, y_train, y_test, intParams,
                 bounds, features_list=None):
        ...
        if self.fs:
            ...
            #adaboost parameters
            #n_estimators, n_trees, typical range [10,5000]

            self.lb[self.nfeatures] = bounds['lb_n_estimators'] #lower bound of n_estimators, integer [10,5000]
            self.ub[self.nfeatures] = bounds['ub_n_estimators']  # upper bound of number of estimators, integer

            #base, ovo je bazni klasifikator, by default je decision_tree classifier, sada biramo njegov depth, integer, [1,11]

            self.lb[self.nfeatures + 1] = bounds['lb_depth']  # lower bound of depth of base classifier Decision Tree
            self.ub[self.nfeatures + 1] = bounds['ub_depth']  # upper bound of depth of base classifier Decision Tree

            self.lb[self.lr + 2] = bounds['lb_lr']  # lower bound of learning rate, real [0.1,2]
            self.ub[self.lr + 2] = bounds['ub_lr']  # upper bound of learning rate
            ...
            self.lb[self.nfeatures] = bounds['lb_n_estimators']  # lower bound of n_estimators, integer
            self.ub[self.nfeatures] = bounds['ub_n_estimators']  # upper bound of number of estimators, integer
            ...
            self.lb[self.nfeatures + 2] = bounds['lb_lr']  # lower bound of learning rate, real [0.1,2]
            self.ub[self.nfeatures + 2] = bounds['ub_lr']  # upper bound of learning rate

        #print('self.y_train', self.y_train)
        #print('self.y_train1', self.y_train1)
    ...
    def function(self, x):
        ...
        #xgb_clf = xgb.train(params, self.d_train)
        #y_proba = xgb_clf.predict(self.d_test)
        adaboost_classifier = AdaBoostClassifier(n_estimators=int(round(x[self.nfeatures], 0)),
                                                 estimator=DecisionTreeClassifier(max_depth=int(round(x[self.nfeatures+1], 0))),
                                                 learning_rate=x[self.nfeatures+2],algorithm='SAMME')



        adaboost_classifier.fit(self.x_train,self.y_train1)
        y_proba = adaboost_classifier.predict_proba(self.x_test)
        ...
        mcc = matthews_corrcoef(y_cappa, self.y_test1)

        # posto idemo na minimizaciju prvo je 1-cohen_kappa
        if self.fitness_type == 0:
            return ((1 - mcc), error, y_proba, y, feature_size, adaboost_classifier)
        else:
            objective = self.alpha * error + self.beta * (feature_size / self.nfeatures)  # ovo je za minimization
            return (objective, error, y_proba, y, feature_size, adaboost_classifier)
